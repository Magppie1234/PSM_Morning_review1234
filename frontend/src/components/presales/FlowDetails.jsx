...
const columnsFor = (cardId) => [
  ['Lead', (lead) => <RecordLink row={lead} />],
  ['PSM', (lead) => show(lead.psm), 'Zoho lead owner (the PSM field)'],
  ['Time in status', (lead) => <TimeInStatus lead={lead} flagLate={!NO_FLAG.has(cardId)} />, 'How long the lead has been in its current Zoho status; red after 24 hours'],
  ['Last contacted', (lead) => <LastContactCell lead={lead} />, 'Latest call logged in Zoho (made, received or missed); scheduled calls are not counted'],
  ['Attempts', (lead) => <AttemptsCell lead={lead} />, 'Outgoing calls placed on the lead, out of the 15 allowed'],
  ['Created', (lead) => <CreatedCell lead={lead} />, 'When the lead was created in Zoho (IST)'],
  ['Modified', (lead) => <ModifiedCell lead={lead} />, 'Last change to the lead in Zoho, its current status and who made it'],
...
const droppedColumns = [
  ['Lead', (lead) => <RecordLink row={lead} />],
  ['PSM', (lead) => show(lead.psm), 'Zoho lead owner (the PSM field)'],
  ['Time in status', (lead) => <TimeInStatus lead={lead} flagLate={false} />, 'How long the lead has been dropped'],
  ...
  ), 'Reason for Cold in Zoho (or Dead Reason when that is empty)']
];
...
const opportunityColumns = (cardId) => [
  ['Client', (row) => <RecordLink row={row} />],
  ['PSM', (row) => show(row.psm), 'Sales Manager field on the opportunity (the PSM who qualified it)'],
  ['Validation', (row) => row.status, 'Client Status in Zoho: blank or Not Yet Validated means the SM has not validated it'],
  ['Value', (row) => show(row.value), 'Total Opportunity Value in Zoho'],
  cardId === 'closed'
    ? ['Closed on', (row) => show(row.closedOn), 'Actual Closure Date in Zoho']
    : ['Last contacted', (row) => <LastContactCell lead={row} />, 'Latest call logged in Zoho'],
  ['Created', (row) => <CreatedCell lead={row} />, 'When the opportunity was created in Zoho (IST)'],
  ['Modified', (row) => <ModifiedCell lead={row} />, 'Last change to the opportunity in Zoho and who made it']
];
...
export function FlowDetails({ card, node, leads, onClose, formula }) {
  ...
  // Longest wait first, so the most overdue leads lead the table.
  const records = [...bySource].sort((a, b) => (statusAge(b) ?? 0) - (statusAge(a) ?? 0))
    .filter((lead) => matchesFocus(lead, focus));
  ...
  return (
    ...
    <section className={`lf-detail tone-${card.tone}`} role="dialog" aria-modal="true" aria-label={`${card.label} records`}>
      <header className="lf-detail-head">
        <div>
          <h3><i aria-hidden="true" />{card.label} <span>{source === ALL && !focus ? node.count : `${records.length} of ${node.count}`}</span></h3>
      ...
      {view === 'chart' ? (
        <LeadPies leads={bySource} focus={focus} onFocus={pickFocus} />
      ) : records.length === 0 ? (
        <p className="lf-detail-empty">{source === ALL ? 'No leads in this group for the selected period.' : `No ${source} leads in this group.`}</p>
      ) : (
        <div className="lf-detail-body">
          <table className="ps-table lf-detail-table ams-stack-table">
            ...
            <thead>
              <tr>{columns.map(([label, , title]) => <th scope="col" key={label} title={title}>{label}</th>)}</tr>
            </thead>
            <tbody>
